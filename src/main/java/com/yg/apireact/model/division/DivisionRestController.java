...
import com.yg.apireact.model.user.UserRestController;
...
public class DivisionRestController {
	...
	public ResponseEntity<List<Division>> getDivisions() {
		...
		try {
			...
			if (iterable == null) {
				log.debug("DivisionRestController -> getDivisions -> No Divisions found.");
				return new ResponseEntity<>(HttpStatus.NO_CONTENT);
			}
			List<Division> response = Streamable.of(iterable).filter(code -> (!code.getCode().equals("0"))).toList();
